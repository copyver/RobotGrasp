...
cv::Mat GrabData::getCP(cv::Mat &srcImage, cv::Mat &ImageDealt) {
    ...
    for (int i = 0; i < contours.size(); i++) {
        //epsilon==1.5
        approxPolyDP(cv::Mat(contours[i]), contours_poly[i], 2.2, true);
        drawContours(dstImage_3, contours_poly, i,
                     cv::Scalar(230, 130, 255), 1, cv::LINE_AA);
    }
...
{
    cv::Mat imdepth = depthImage.clone();
    ...
    float sumDepth = accumulate(begin(depthFinal), end(depthFinal), 0.0);
    meanDepth = sumDepth / depthFinal.size();
    return meanDepth;
}
